  ...
  return (
    <section id="skills" className="py-16 section-enter" ref={sectionRef}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          <h3 className="text-2xl font-semibold mb-4 text-green-400">Technical Skills</h3>
          <ul className="space-y-2">
            {technicalSkills.map((skill, index) => (
              <li key={index} className="flex items-center bg-gray-800 p-3 rounded-lg neon-border">
                {skill.icon}
                {skill.name}
              </li>
            ))}
        ...
        <div>
          <h3 className="text-2xl font-semibold mb-4 text-green-400">Soft Skills</h3>
          <ul className="space-y-2">
